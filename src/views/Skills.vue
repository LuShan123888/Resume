<template>
  <div>
    ...
    <div :class="this.class.title" :style="{ color: $vuetify.theme.themes.light.primary }" v-text="title">
    </div>
        ...
        <v-list>
          <v-list-item-group>
            <div v-for="(item, i) in info" :key="i">
              <v-list-item>
                <v-list-item-content>
                  <span style="font-weight: bold;line-height: 150%;"><span
                      :style="{color:$vuetify.theme.themes.light.primary}">{{i+1}}. </span>{{item}}</span>
                </v-list-item-content>
              </v-list-item>
...
<script>
  ...
  import bgImg from "../assets/img/coding.svg";
  export default {
    name: "Home",
    ...
    mounted() {
      this.axios
        .get("/test.json")
        .then((response) => (this.info = response.data.skills));
    },
  };
</script>
